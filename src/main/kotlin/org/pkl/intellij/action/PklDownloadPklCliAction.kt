...
class PklDownloadPklCliAction(private val onStart: () -> Unit, private val onEnd: (String?, Throwable?) -> Unit) :
  DumbAwareAction("Download Pkl Executable", "Downloads Pkl executable", AllIcons.Actions.Download) {
  ...
  private fun downloadPklExecutable(targetPath: Path, url: String) {
    ...
    runBackgroundableTask("Downloading Pkl executable") {
      try {
        ...
        URL(url).openStream().use { input -> Files.copy(input, targetPath, StandardCopyOption.REPLACE_EXISTING) }
  ...
  private class DownloadPklDialog(project: Project) : DialogWrapper(project) {
    ...
    override fun createCenterPanel(): JComponent = panel {
      ...
      row("Pkl version:".wrapInCode()) {
        combo = comboBox(emptyList<String>()).onChanged {
      ...
      row("Destination path:".wrapInCode()) {
        textField = textField().bindText(::destinationPath)
      }
    ...
    fun fetchAvailableVersions(onEnd: (List<String>) -> Unit) {
      ...
      val versionElements = document.select("li[class='nav-item']")
        .mapNotNull { Regex("""(\d+(\.\d+)*?) Release Notes""").find(it.text())?.groups?.get(1)?.value }
        .distinct().sortedDescending().map { "$it.0" }
