  ...
  override fun doAnnotate(element: PsiElement, holder: AnnotationHolder) {
    if (element is PklNewExpr) {
      ...
      val definedProperties =
        element.objectBody?.properties?.let { getAllDefinedProperties(it, base, context) }
          ?: emptyMap()
      val missingProperties = requiredProperties.minus(definedProperties.keys)
...
private fun insertRequiredProperties(
  ...
  props.forEach {
    ...
    val hasDefault =
      when (propertyType) {
        is Type.Module -> false
        is Type.Class -> false
        else -> propertyType.hasDefault(base, context)
      }
...
private fun getAllDefinedProperties(
  props: Sequence<PklObjectProperty>,
  base: PklBaseModule,
  context: PklProject?
): Map<String, String> {
  return props.associateBy(
    { it.propertyName.identifier.text },
    {
      when (val bodyType = it.getLookupElementType(base, mapOf(), context)) {
        is Type.Class -> bodyType.psi.displayName
        is Type.Module -> bodyType.psi.displayName
        else -> bodyType.render()
      }
    }
  )
}
