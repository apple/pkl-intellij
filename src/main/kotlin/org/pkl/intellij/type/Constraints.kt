...
import org.pkl.intellij.type.ConstraintExpr.*
import org.pkl.intellij.type.ConstraintValue.*
import org.pkl.intellij.util.escapeString
import org.pkl.intellij.util.isMathematicalInteger
  ...
  return when (this) {
    ...
    is PklNumberLiteral -> {
      ...
      when {
        ...
        else -> strippedText.toLongOrNull()?.let { IntValue(it) }
            ?: strippedText.toDoubleOrNull()?.let { FloatValue(it) } ?: Error
      }
    }
    is PklStringLiteral -> content.escapedText()?.let { StringValue(it) } ?: Error
    is PklMlStringLiteral -> content.escapedText()?.let { StringValue(it) } ?: Error
    is PklTrueLiteral -> True
    is PklFalseLiteral -> False
    is PklNullLiteral -> Null
    is PklThisExpr -> ThisExpr
    is PklTypeTestExpr -> TypeTest(expr.toConstraintExpr(base), type.toType(base, mapOf()), base)
    is PklIfExpr ->
      If(
            ...
            base.intType.psi ->
              when (resolved) {
                ...
                base.intIsOddProperty -> IntIsOdd(receiverExpr)
                  ...
                  val quantity: Double =
                    when (@Suppress("UnnecessaryVariable") val recvExpr = receiverExpr) {
    ...
    is PklBinExpr -> {
      val leftExpr = this.leftExpr.toConstraintExpr(base)
      val rightExpr = this.rightExpr.toConstraintExpr(base)
      when (this) {
        is PklEqualityBinExpr -> {
          when (operator.elementType) {
            PklElementTypes.EQUAL -> Equals(leftExpr, rightExpr)
            PklElementTypes.NOT_EQUAL -> NotEquals(leftExpr, rightExpr)
            else -> Error
          }
        }
        is PklComparisonBinExpr -> {
          when (operator.elementType) {
            PklElementTypes.LT -> LessThan(leftExpr, rightExpr)
            PklElementTypes.LTE -> LessThanOrEqualTo(leftExpr, rightExpr)
            PklElementTypes.GT -> GreaterThan(leftExpr, rightExpr)
            PklElementTypes.GTE -> GreaterThanOrEqualTo(leftExpr, rightExpr)
            else -> Error
          }
        }
        is PklLogicalAndBinExpr -> And(leftExpr, rightExpr)
        is PklLogicalOrBinExpr -> Or(leftExpr, rightExpr)
        is PklNullCoalesceBinExpr -> NullCoalesce(leftExpr, rightExpr)
        else -> Error
      }
    }
